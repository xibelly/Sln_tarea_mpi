/* Xibelly Eliseth Mosquera Escobar - Lunes 21 de Nov de 2016
 ...
 2) Escribir un programa que utilice la serie de taylor de la funcion cos(alrededor de x=0) para calcular el cos de un angulo. El programa debe recibir como entrada el valor del angulo (x) en grados y en el numero de terminos (N) de la serie a usarse en la expansion, y retornar en pantalla el valor de cos(x). Debe de correr con un # indeterminado de procesadores.
...
Para resolver el problema de calcular el cos de un # usando la serie de taylor centrada en x=0
  ...
  cos(x) = sum_n=0^inf ( (-1)^n x^{2n+1} )/ (2n+1)!                                 (1)
...
int main(int argc, char **argv)
{
  ...
  double angulo, numerador, term, denominador, suma=0.0, cos_x, radianes, suma2=0.0;
  double  term2, numerador2,denominador2, cos2_x;
